...
| T08 | SPA route serving | Render the same composed Jinja template at `/trips`, `/expeditions`, `/stats`, `/map`, `/gear`, `/gallery`, and `/settings`; `/` selects the Trips view. | Implemented | Browser URL/nav | Flask template rendering | `server.py`, `templates/index.html`, `app.js` | None | Page routes | Routed screens |
