...
| T08 | SPA route serving | Serve the same `index.html` at `/trips`, `/expeditions`, `/stats`, `/map`, `/gear`, `/gallery`, and `/settings`; `/` redirects to `/trips`. | Implemented | Browser URL/nav | Flask static serving | `server.py`, `app.js` | None | Page routes | Routed screens |
