...
async function uploadImageFile(file, category, metadata = {}) {
  ...
  const response = await fetch(apiUrl(`/api/uploads/${category}`), {
    method: "POST",
    body: formData
  });
  if (!response.ok) {
...
async function loadPhotoQueue() {
  const response = await fetch(apiUrl("/api/photo-queue"));
...
async function claimQueuedPhoto(filename) {
  ...
  try {
    const response = await fetch(apiUrl("/api/photo-queue/claim"), {
      method: "POST",
...
async function deleteQueuedPhoto(filename) {
  try {
    const response = await fetch(apiUrl(`/api/photo-queue/${encodeURIComponent(filename)}`), { method: "DELETE" });
