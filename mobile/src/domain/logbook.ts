...
export function tripDurationHours(trip: Trip): number { const startTime = trip.linesSetTime || trip.startTime, endTime = trip.linesPulledTime || trip.endTime; if (!trip.date || !startTime || !endTime) return 0; const start = new Date(`${trip.date}T${startTime}`); const end = new Date(`${trip.date}T${endTime}`); if (end < start) end.setDate(end.getDate() + 1); return Math.max(0, (end.getTime() - start.getTime()) / 3_600_000); }
