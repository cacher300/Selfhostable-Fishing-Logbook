...
export function normalizeLogbook(input: Partial<Logbook> | null | undefined): Logbook {
  ...
  logbook.settings = { ...defaultLogbook.settings, ...(input?.settings ?? {}) }; logbook.trips = logbook.trips.filter(Boolean).map(raw => { const trip = normalizeTrip(raw); return { ...trip, catches: trip.catches.map(fish => assignCatchSpot(fish, logbook.spots)) }; }); return logbook;
}
